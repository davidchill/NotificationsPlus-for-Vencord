...
- **Friend highlighting** — new `Friend Highlight` section in settings. Toasts from people on your friends list can be distinguished three independent ways: a star badge beside the sender name (`Star badge by the name`, default on), an accent swap that recolors the whole toast — border, glow, channel line and badge — to a configurable color (`Recolor toast accent`, default on, `#f0b232`), and a text prefix on the name (`Prefix the name with a label`, default off). Each is its own toggle under the master `friendHighlightEnabled` switch, so they mix freely. Because the main process only ever sees a `toastXml` blob with no author ID, friend status is resolved renderer-side via `RelationshipStore.isFriend` and pushed to main keyed by message ID (`noteFriendStatus`), where `processNotification` resolves it from the ID returned by `extractNavData`. Best-effort by design: if the note has not arrived by the time the toast builds, the toast simply renders without the highlight. The settings panel's test button also fires a friend DM preview so the styling can be checked without waiting for a real friend to message.
- **`icon-cache` diagnostic scope** — instruments the avatar LRU with a running hit/miss ratio, current entry count, and the resolved temp path. On a HIT it also stats the file and compares its byte size against what was cached, flagging `STALE` on a mismatch. This exists to settle a question the code cannot answer on its own: whether Discord reuses avatar temp paths across notifications. Stable per-user paths mean the cache earns its keep; unique-per-notification paths mean the 50 entries are dead weight; a path reused for a *different* avatar would mean serving a stale face, which is a correctness bug rather than a performance one. Entirely gated on `npDebug`, including the extra `stat` syscall.

### Performance

- **Font CSS moved off the per-toast hot path** — `ensureFontCached` inlines every weight and unicode subset as base64. For Nunito (the default, `wght@400;500;600;700`) that is 20 `.woff2` files across 5 subsets: 537 KB raw, **~716 KB as a base64 string**. That string was carried as `fc` inside every `UpdateData`, so each toast structured-cloned ~716 KB to the toast renderer, which then assigned it to `<style id="fs">.textContent` — forcing a full CSSOM reparse *before* the height-measurement reply that `win.show()` waits on. It is now pushed once per window over a new `np:font` IPC channel (`applyFont` in the preload, `window.__npFont` in the inline fallback), fired from `createPoolWindow` as soon as `loadURL` resolves, so the reparse happens during background pool warm-up where nothing is waiting on it. The per-toast payload drops to roughly 2 KB. A `WeakMap` stamp of `` `${fontName}:${cssLength}` `` keeps the hot-path check to a comparison and an early return, while still correctly re-pushing to a window warmed before the download finished (stamp `Nunito:0`) or before a font change; `refreshPoolFonts()` handles the latter on every `updateMainProcessPatch`. Note this relocates the cost rather than removing it — each toast still consumes a fresh window, so the bytes still move once per toast, just off the critical path. Recycling windows would reduce it to once per window lifetime.
- **Friend-status IPC reduced to friends only** — `onMessageCreate` fired `Native.noteFriendStatus(...)` for every `MESSAGE_CREATE` anywhere in the account, and Vencord's `pluginHelpers` is built on `ipcRenderer.invoke`, so each one was a full renderer to main and back round trip plus a floating promise — for messages that would almost never become a notification. Only friends are pushed now. Behavior is identical because main reads the map with `.get()`, where a missing entry and `false` both collapse to `false`. Side benefit: main's 200-slot `FRIEND_STATUS_MAX` LRU now holds only friend entries, so non-friend traffic can no longer evict a real friend entry before the toast builds — a window that is widest exactly when coalescing is holding the toast behind a burst.
- **Diagnostic strings no longer built when `npDebug` is off** — `logDiag` checked `debugEnabled` internally, but the template literal was evaluated at the call site regardless. Five hot-path sites are now guarded with `if (debugEnabled)`: `burst-skip`, `evict`, `toast.show` (four `toFixed()` calls plus a four-way concat, on every toast), `pool` (which also eagerly called `targetPoolSize()`), and `coalesce-flush`. The four `performance.now()` marks in `showToastInternal` are deliberately left unconditional — they cost nanoseconds, and making them conditional would emit one garbage timing line for whichever toast is in flight when the setting is toggled.
- **Display list cached** — `screen.getAllDisplays()` re-enumerates monitors on every call, and `showToastInternal` hit it on every toast plus once more on the burst-skip path. A new `getDisplaysCached()` caches the array and invalidates on `display-added`, `display-removed` and `display-metrics-changed` — the last being the one that matters most here, since it fires on taskbar, DPI and resolution changes, which is exactly what moves `workArea`. Listeners bind lazily on first use so correctness does not depend on which entry point runs first, and unbind in `stopMainProcessPatch`. The settings-panel `getDisplays()` export stays deliberately uncached: it runs rarely and should always reflect whatever monitors are plugged in right now.

### Bundle checklist (this release)
- `native.ts` — `np:font` push (`appliedFont` stamp, `pushFontIfStale`, `refreshPoolFonts`), `fc` removed from `UpdateData`/`buildUpdateData`, friend-status LRU + `noteFriendStatus`, friend fields on `ToastOptions`, `logDiag` call-site guards, `getDisplaysCached` + listener bind/unbind, `icon-cache` instrumentation + `IconCacheEntry`
- `toastTemplate.ts` — `FRIEND_SVG`, `.title-row`/`.fbadge` styles and markup, `applyFont` + `np:font` handler in `PRELOAD_SRC`, `window.__npFont` in `TEMPLATE_HTML`, `d.fc` removed from both `applyUpdate` and `__npUpdate`
- `index.tsx` — friend highlight settings and panel section, `RelationshipStore` lookup, friends-only `noteFriendStatus` push, friend fields in `getToastShared`, friend DM preview in the test button
- `.gitignore` — `CLAUDE.md`
- `CHANGELOG.md` — this entry
- `README.md` — version bump, Friend Highlight settings section, font handling rewritten, pool refill wording corrected, `icon-cache` added to the diagnostics list
