...
- `toast.show` — per-toast pipeline timing (`acquire`, `update`, `tail`, `total`, `measuredH`). `acquire` is broken down as `[pre= icon= create= ready=]`, where `acquire = pre + max(icon, create + ready)`: `pre` is work before the pool is touched (burst-skip, eviction, the DM group window), `icon` is the avatar read, `create` is the synchronous pool refill, and `ready` is the wait for the handed-out window's `loadURL`
